...
class MainUtils:
    ...
    def load_numpy_array_data(self, file_path: str) -> np.array:
        logging.info(
            "Entered the load_numpy_array_data method of MainUtils class")
        try:
    ...
    def get_tuned_model(
        ...
        try:
            ...
            logging.info(
                "Exited the get_tuned_model method of MainUtils class")
    ...
    def get_model_score(test_y: DataFrame, preds: DataFrame) -> float:
        ...
        try:
            ...
            logging.info(
                "Exited the get_model_score method of MainUtils class")
            return model_score
    ...
    def get_base_model(model_name: str) -> object:
        ...
        try:
            if model_name.lower().startswith("xgb") is True:
                ...
                model_idx = [model[0]
                             for model in all_estimators()].index(model_name)
    ...
    def get_model_params(
        ...
        try:
            ...
            logging.info(
                "Exited the get_model_params method of MainUtils class")
    ...
    def create_artifacts_zip(file_name: str, folder_name: str) -> None:
        logging.info(
            "Entered the create_artifacts_zip method of MainUtils class")
        try:
            shutil.make_archive(file_name, "zip", folder_name)
            logging.info(
                "Exited the create_artifacts_zip method of MainUtils class")
    ...
    def update_model_score(self, best_model_score: float) -> None:
        logging.info(
            "Entered the update_model_score method of MainUtils class")
        try:
            model_config = self.read_yaml_file(filename=MODEL_CONFIG_FILE)
            ...
            logging.info(
                "Exited the update_model_score method of MainUtils class")
